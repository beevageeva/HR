...
class CanvasFrame(wx.Frame):
		...
		def __init__(self, mydatafile, iniisofiles):
				...
				self.toolbar.update()
				for filename in iniisofiles:		
					self.addIsochroneFile(filename)
		...
		def plotMyData(self):
			mydata = np.genfromtxt(self.mydatafile, comments="#")
			self.axes.plot(mydata[:,0],mydata[:,1], "ko", markeredgecolor='none', markersize=1, label="My Data")
			#self.axes.plot(mydata[:,0],mydata[:,1], "ro", markeredgecolor='none', color="#000000" , markersize=1)
			self.axes.set_xlabel('Mv - Mi')
			self.axes.set_ylabel('Mi')
...
class App(wx.App):
		...
		def OnInit(self):
				...
				argv = sys.argv[1:]
				mydatafile="dhr7.dat"
				iniisofiles  =   ["isocz0004.dat" , "isocz001.dat",  "isocz004.dat" , "isocz008.dat", "isocz019.dat", "isocz030.dat"]
				try:
					opts, args = getopt.getopt(argv,"hi:l:",["ifile=","lfile="])
				except getopt.GetoptError:
					print 'wxhr.py -i <inputfile> -l <iniisofilelist>'
					sys.exit(2)
					for opt, arg in opts:
						if opt == '-h':
							print 'wxhr.py -i <inputfile> -l <iniisofilelist>'
							...
							listfile = arg
							f = open(listfile,'r')
							iniisofiles = f.readlines()
							f.close()	
				print 'mydata file is ', mydatafile
				print 'iniisofiles files: ', str (iniisofiles)
				frame = CanvasFrame(mydatafile, iniisofiles)
				frame.Show(True)
